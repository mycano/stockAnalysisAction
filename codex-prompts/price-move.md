---
name: "price-move"
description: "已弃用的 /price-move 兼容入口；转发到 /move。"
...
此入口自 4.17.0 起弃用，仅兼容转发到 `/move`；不得复制或执行第二套
业务协议。向用户显示弃用提示，保留原参数，并将其整理为：

```json
{"schema_version":"2.0","command":"move","arguments":{"window_type":"single-session","asset":"<asset>"}}
```
